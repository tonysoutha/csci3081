...
|:----:|:-----|:------------|
| TUE, Oct 22, 11:55pm | UML diagram of proposed solution | submit via Github |
| WED, Oct 30, 11:55pm | Preliminary testing submission   | Pass automated tests |
| WED, Nov 06, 11:55pm | Documentation and Code Implementation | Automated tests and inspection |

Your project should demonstrate thoughtful software development with good design quality while rigourously following the process you learned in lab 10. This iteration will help you to establish those good habits. You will create design documents including a UML diagram. You will use_doxygen_ compliant comments to comment your code and then automatically generate code documentation. Code style will comply to the Google Style Guide. Intermediate deadlines will keep you on track in the iterative process you are using to develop iteration 1, and the project this semester.
...
The goal of this software project is to develop a rudimentary transit simulator in which simluations can be controlled via external configuration (without code change) and be visualized within a graphics window. 
...
Before discussing various levels of priority for our domain entities, there is one method which requires a special note.
...
#### Priority Level 1 : Passengers and Stops

##### Passenger

Be sure to confirm that all values of the Passengers state are accurate after your methods execute. Particularly, that the Passenger counts all of the time steps in the simluation from the time step it is generated to the time step it gets off of its bus at its destination (both inclusive).

There are three methods which warrant further discussion.

###### _ Update _

All of our domain objects will Update. This method must make whatever changes to state that are required of the simluation. When the simulation is running, a passenger is doing one of three things for that time step: waiting at a stop, riding on a bus as it moved, or getting on/off the bus.

###### _ Get Total Wait _

Returns the sum total of all times steps it has waited at a stop before getting on the bus. This time does *NOT* include the timestep during which is gets on a bus.
...
Represents each place a bus stops to pick up and drop off passengers. It should be noted that when a real-world stop on a bus schedule has stopping locations on opposite sides of the street, we treat these as two different Stops in our vernacular. The default location is the Coffman Memorial Union stop, for now.
...
#### Priority Level 2 : Routes and Output Refactoring

##### Routes

Routes are sets of stops. This makes sense in the real world and in our system. As such, our current idea of a route is to have an instance hold references to the stops included in its route as well as the distances between each stop. The current understanding is that each Route differs slightly from the real-world of a "route" in that our Route are always one-way. This is partially to simplify the model. Having passengers show up at a stop and could be going in either direction makes the Stop concept more complicated. When displaying the stops, it is far easier to display by keeping each direction of the route as a distinct concept. For example, in this first iteration, you can see that we have an eastbound route for the campus connector as well as a westbound route.

##### Output Refactoring

In early prototyping and debugging of individual units, it is very common to simply use the display for output of information. std::cout is a familiar friend in this. However, once we get to a point where we want to have a persistent access to results, flexibility in operation and easier ability to test program output. To this end, we will refactor our code to allow for the use of other ostream objects in place of std::cout for the Report() methods that exist within our system.

Find each instance of this method and refactor the code to include the ability to give the method an instance of an ostream object (which may or may not be std::cout in the end). Remember: refactoring means to modify the internal functionality of the code while maintaining the same observable behavior outside the system. Our methods should still work the same way from the outside (assume that our simulator will be similarly updated to use this new functionality. You may also do this yourself in your testing.

#### Priority Level 3 : Buses

Now that the other three are there, we can build our final domain object, the Bus.  Buses move and pick up/drop off passengers. To that end, our buses have a speed (a distance to move per time unit) and a max capacity for passengers. Glaringly absent is how the bus keeps track of the passengers on that bus. In addition, our two common methods (Update and Report) are missing as well. Each of these will need to be added for this system to work well.

#### Challenge [NOTE: This is UNGRADED, only for fun.]

Extend the local simluation with command line control of the time between bus generations. Allow the user to place an integer value as an argument when invoking the simluation from the command line. Use this integer to control the bus generation in LocalSimulator.
...
Unit tests are essential in a large-scale project, because the entire code base can be tested regularlyand automatically
as it is being developeed. In the strictest application of Test-Driven Development (TDD), the tests are written
...
#### Preliminary Testing Submission ( 10% )
